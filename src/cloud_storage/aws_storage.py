...
class SimpleStorageService:
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = False):
        ...
        try:
            ...
            if not from_filename.endswith(to_filename):
                file_path = os.path.join(from_filename, to_filename)
            else:
                file_path = from_filename
            print("File Path: ", file_path)
            with open(file_path, 'rb') as f:
                self.s3_client.put_object(Bucket=bucket_name, Key=to_filename, Body=f)
