        ...
        try:
            ...
            if MODEL_ENV == "aws":
                ...
                model = Proj1Estimator(
                    bucket_name=self.prediction_pipeline_config.model_bucket_name,
                    model_path=self.prediction_pipeline_config.model_file_path,
                    model_path_local=self.prediction_pipeline_config.trained_model_file_path
                )
                result = model.predit_local_model(dataframe)
            else:
                raise ValueError("Invalid model environment")
            return result
